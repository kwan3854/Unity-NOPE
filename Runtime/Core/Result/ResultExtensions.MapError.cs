...
namespace NOPE.Runtime.Core.Result
{
    ...
    {
        /// <summary>
        /// If the Result is failure, applies a function to the error to produce a new error.
        /// Otherwise, returns the original success.
        /// </summary>
        public static Result<T, E> MapError<T, E>(
            this Result<T, E> result,
            Func<E, E> errorSelector)
        {
            if (result.IsFailure)
            {
                var transformed = errorSelector(result.Error);
                return Result<T, E>.Failure(transformed);
            }
            return result;
        }
        ...
        /// Otherwise, returns the original success.
        /// </summary>
        public static async UniTask<Result<T, E>> MapError<T, E>(
            this Result<T, E> result,
            Func<E, UniTask<E>> errorSelectorAsync)
        {
            if (result.IsFailure)
            {
                var newErr = await errorSelectorAsync(result.Error);
                return Result<T, E>.Failure(newErr);
            }
            return result;
        }
        ...
        /// Otherwise, returns the original success.
        /// </summary>
        public static async Awaitable<Result<T, E>> MapError<T, E>(
            this Result<T, E> result,
            Func<E, Awaitable<E>> errorSelectorAwaitable)
        {
            if (result.IsFailure)
            {
